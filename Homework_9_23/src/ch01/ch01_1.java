...
/*       1번문제 JDK와 JRE의 차이점 중 틀린 것은?
	 		1.JDK는 JRE와 컴파일러 등의 개발 도구가 포함된다.
			2.자바 프로그램을 개발하려면 JDK가 반드시 필요하다.
			3.자바 프로그램을 실행만 하려면 JRE를 설치해도 상관없다. 
			4.JRE에는 컴파일러(javac.exe)가 포함되어 있다. -> 답
			설명 JRE = JVM + 라이브러리 API  JDK = JRE + 개발에 필요한 도구(컴파일러 등)

		 2번문제	JVM에 대한 설명으로 틀린 것은?
			1.JVM은 java.exe 명령어에 의해 구동된다.
			2.JVM은 바이트 코드를 기계어로 변환시키고 실행한다.
			3.JVM은 운영체제에 독립적이다(운영체제별로 동일한 JVM 사용)
			4.바이트 코드는 JVM에 독립적이지만, JVM은 운영체제에 종속적이다. ->답
			5.JVM은 운영체제마다 다르게 설치된다.
			JRE나 JDK를 설치하면 자동으로 설치하는데, 이 두가지가 운영체제 별로 제공된다.

	     3번문제	자바 프로그램 개발 과정을 순서대로 나열하세요.
		...
		/*	이클립스에 대한 설명으로 틀린 것은?
			...
			4.metadata는 프로젝트의 소스 파일들이 저장되는 디렉토리이다 ->답
			설명:metadata는 이클립스를 사용하면서 변경되는 속성값이 기록되는 디렉토리이다.*/
	}
